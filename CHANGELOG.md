...
- **Client (UI) / Server**: Show colour coding for G-Stand dose rule checks in the Formulary view — server computes a `DoseCheckColor` status per dose rule check result; Formulary renders each check with a colour indicator so clinicians can immediately distinguish compliant, borderline, and exceeded dose limits; `DoseCheckColor.fsx` prototype and `DoseCheckColor.migration.md` added to `src/Informedica.GenPRES.Server/Scripts/`; 127 new server-side tests (PR #309)

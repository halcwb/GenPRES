...
- **Server**: Close XFF bypass on rate limiter — rate-limiter now correctly ignores attacker-controlled `X-Forwarded-For` headers, closing a bypass vector identified in PR #298 review; adds a security ADR documenting the mitigation (PR #298)
- **Server**: Fix CSS Content Security Policy headers — resolve CSP violations in security header configuration (PR #298)
- **Docs (security)**: Update security documentation and remove unused package — security review document updated to reflect implemented fixes; unused dependency removed (PR #299)

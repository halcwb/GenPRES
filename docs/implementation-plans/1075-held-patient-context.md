...
[UC-12](../scenarios/integration/uc-12-held-patient-context.md); this plan builds it.
...
A change to the patient context after an order is on the plan re-evaluates the plan's totals
and leaves each order as it was computed. The plan's patient, its totals and the signed
version's patient data follow the edit; nothing compares them with the patient each order was
computed on. So a User can sign orders composed on different data, and the version says they
rest on the last of it.

## What the code does today

- **Client.** `updatePatient` in `App.fs` sends `PatientChanged` to the order context and the
  plan. `OrderPlanMachine.step` puts the new patient on the plan and asks `Recalculate`; the
  contexts keep the patient they were created with. `PlanWork` (`PlanWorkPolicy.fs`) counts the
  commands that changed the plan since the version last opened or signed, for the guard that
  asks before leaving the page; it does not say which contexts are new or changed.
- **Server.** Every plan context carries the patient it was computed on
  (`PlanContext.Context.Patient`). `Session.challenge` and `Session.commit` check the Session,
  the Role, the token, the head and the challenge, and refuse a plan that names an order twice;
  nothing compares a context's patient with the plan's. The Server keeps nothing of the plan
  between requests.
...
**Chosen: derived from the plan, by the Client for the panel and by the Server at the
signature.** Only the order plan is signed, as a version. A new or changed context is one the
version last opened or signed does not hold as it
is. The Client knows that version and the plan; the Server gets both at the challenge and the
commit, the head from its store and the plan from the request.

- *A row when the first new or changed order arrives*: rejected. The Server would keep a fact
  about the plan it otherwise never keeps, and still has to check the plan at the signature,
  since the row cannot say what the plan holds.
...
**Chosen: every new or changed context carries the plan's patient context**, compared on the
data the rules read. Contexts the head holds unchanged are exempt: they are part of a signed
...
- *Against the Session's patient instead of the plan's*: the plan's patient is what the version
  is signed on, and the Session's is already put on the plan's age; comparing with the plan's
  keeps the check a function of the request and the head alone.
...
- *Refuse the sign until the User drops the new and changed orders*: rejected; the EHR moving
  must not make a correct plan unsignable.

### The age

**Chosen: unchanged.** The #976 rule holds the age from the open to the sign whatever the plan
holds. Letting it follow the clock while released would change a rule that just landed, for a
case the idle end of #1061 already bounds.

## Chosen approach

- **Hold** (Client.Core): the plan state keeps the contexts of the version last opened or
  signed; `held` is whether the plan has a context that version does not hold as it is. Pure,
  tested under Expecto.
...
- **Check** (Server): at the challenge and the commit, a plan whose new or changed contexts
  carry another patient context than the plan's is refused `ContextDiffers`, a new
  `SigningRefusal`. The comparison is on the data the rules read.
...
Medium. The check at the signature is a pure function of the plan and the head and carries the
safety of the change. The Client's hold needs the plan state to keep the version it opened or
signed, which it does not today. The refresh adds a Session command and a second place the EHR
is read.
...
1. **The hold (script).** `src/Informedica.GenPRES.Client.Core/Scripts/HeldContext.fsx`:
   the plan state keeps the contexts of the version last opened or signed; `changed` names the
...
4. **The check (script).** `src/Informedica.GenPRES.Server/Scripts/HeldContext.fsx`: the
   patient context of a plan context compared with the plan's on the data the rules read;
   `changed` against the head; `challenge` and `commit` shadowed to refuse `ContextDiffers`
   before the challenge is issued and before the PIN is checked.

   Tests over the order plan scenarios: a plan whose orders share the plan's context passes; a
   new order on another weight, department or renal function is refused at both; a head
   order on an older context passes; a plan without a head, every context new, is checked
   whole; a refusal costs no PIN attempt.
...
| 5 | A plan with a new order on another weight, sent by hand, is refused `ContextDiffers` at the challenge. |
| 6 | With the stub's EHR data changed after the open and an order on the plan, the version is signed on the held data, not verified. |
...
- **Re-prescribing on a new context.** Deferred to #672; until then a new bedside weight means
  signing the plan or dropping its new and changed orders first. Accept the cost, or pull #672
  in.
...
- **#518** the carry-over of a plan's changes into a relaunched tab, which would carry the hold
  with it.
